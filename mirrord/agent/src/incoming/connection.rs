...
impl MaybeHttp {
    ...
    /// Creates a new connection without HTTP detection.
    /// Use this for ports where HTTP detection should be skipped (e.g., server-first protocols).
    pub fn new_tcp(redirected: Redirected) -> std::io::Result<Self> {
        ...
        let local_addr = redirected.stream.local_addr()?;
